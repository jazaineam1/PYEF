-- Final state payload adds counterfactual data ONLY after the reveal gate is opened.
...
begin
  ...
  return jsonb_build_object(
    ...
    'role_card',card,'teams',coalesce(teams_json,'[]'::jsonb),'decision',d,'result',result_json,'locked',locked,'customers',customers,'reveal_payload',reveal_payload,'segments',segments_json
  );
end $$;
...
-- Harden final decision locking: a team gets one irreversible lock per round.
...
begin
  ...
  if p_round=1 then
    ...
    select count(*) into invalid_count from jsonb_array_elements_text(p_payload->'selected') s(id) left join public.cw_ground_truth_customers c on c.customer_id=s.id where c.customer_id is null;
    if invalid_count>0 then raise exception 'Selección contiene clientes inválidos'; end if;
    ...
    select count(distinct id), count(*) filter(where id not in ('digital','middle','traditional','wealth','arrears')) into union_count, invalid_count from all_ids;
...
-- Final facilitator state includes a provisional observed-conversion ranking after R1 closes.
...
begin
  ...
  select jsonb_agg(jsonb_build_object(
    'id',t.id,'name',t.name,
    'online',(select count(*) from public.cw_players p where p.team_id=t.id and p.active and p.last_seen_at>now()-interval '90 seconds'),
    'locked',exists(select 1 from public.cw_decisions d where d.team_id=t.id and d.round_number=g.current_round),
    'observed_conversion',case when g.current_round=1 and g.status in ('closed','reveal','teaching','microcheck','finished') then (
      select round(100*avg(c.p1),1) from public.cw_decisions d
  ...
  return jsonb_build_object('game',jsonb_build_object('id',g.id,'code',g.code,'phase',g.status,'round',g.current_round,'closes_at',g.closes_at,'public_message',g.public_message,'rank_mode',rank_mode),'teams',coalesce(teams_json,'[]'::jsonb),'audit',events_json);
end $$;
...
begin
  ...
  select jsonb_agg(jsonb_build_object(
    ...
    'score',jsonb_build_object('impact',t.score_impact,'evidence',t.score_evidence,'design',t.score_design,'risk',t.score_risk,'adaptation',t.score_adaptation)
  )) into teams_json from public.cw_teams t where t.game_id=g.id;
  return jsonb_build_object('game',jsonb_build_object('code',g.code,'phase',g.status,'round',g.current_round,'closes_at',g.closes_at,'public_message',g.public_message,'rank_mode',rank_mode),'teams',coalesce(teams_json,'[]'::jsonb));
end $$;
