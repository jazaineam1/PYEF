...
Curriculum role cards are seeded by migrations and are intentionally preserved here.
"""
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--code',default=None);ap.add_argument('--pin',default=None);ap.add_argument('--output',default='scenario-private.sql');ap.add_argument('--seed',type=int,default=None)
    a=ap.parse_args();rng=random.Random(a.seed if a.seed is not None else secrets.randbits(64));pin=a.pin or secrets.token_urlsafe(12);code=(a.code or ('FUT-'+secrets.token_hex(3))).upper();rows=[]
