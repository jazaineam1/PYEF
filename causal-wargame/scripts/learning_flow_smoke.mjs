...
// The first four joins are the four teams' Decision Leads, so A/B can operate their own team portfolio.
...
// V6 baseline: transfer check is diagnostic and does not affect leaderboard.
...
for(let round=1;round<=4;round++){
  ...
  if(round===1){
    ...
    const ids=new Set(hsA.data.catalog.map(x=>x.id));assert(hsA.data.catalog.length===5,'V6 market exposes exactly five resources');assert(['basic_hint','round_tool','junior_analyst','senior_specialist','expert_trainer'].every(id=>ids.has(id)),'V6 market resource set mismatch')
    const ja=hsA.data.catalog.find(x=>x.id==='junior_analyst'),jb=hsB.data.catalog.find(x=>x.id==='junior_analyst');assert(ja?.next_cost===6&&jb?.next_cost===6,'each team starts Junior at 6')
    const basket=await call('buy-help-batch',{items:[{help_id:'junior_analyst',qty:2},{help_id:'expert_trainer',qty:1}]},{'x-game-token':ptA});assert(basket.ok&&basket.data.units===3&&basket.data.total_cost===35,'team A basket costs junior 6+9 plus chapter call 20');assert(basket.data.state.investment_balance===25&&basket.data.state.help_cost===35,'team A wallet accounting')
    ...
    const expertSold=await call('buy-help',{help_id:'expert_trainer'},{'x-game-token':ptB});assert(!expertSold.ok,'chapter call remains globally scarce because a human is occupied')
  }
...
console.log('CAUSAL_QUEST_V6_OK · fair per-team market · human scarcity only · R4 uncertainty/capacity · transfer pre/post · 4 rounds complete')
