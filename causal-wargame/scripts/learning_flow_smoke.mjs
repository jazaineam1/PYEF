...
const final=await call('leaderboard',{}, {'x-facilitator-token':ft});assert(final.ok&&final.data.game.phase==='finished','finished phase');assert(final.data.teams.length===5,'five active teams');assert(final.data.teams.every(t=>t.locked),'all five teams submitted final decision');assert(final.data.teams.some(t=>Number(t.help_cost)===35),'team A investment survives to final leaderboard')
const finalWall=await call('leaderboard',{game_code:game});assert(finalWall.ok&&finalWall.data.transfer?.pre_completed>=1&&finalWall.data.transfer?.post_completed>=1,'wall exposes transfer metrics')
