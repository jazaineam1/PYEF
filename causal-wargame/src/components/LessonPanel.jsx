...
function UnlockSummary({round}){
  ...
  return <div className="card unlock-summary"><div className="eyebrow">PODERES QUE SE DESBLOQUEAN CON ESTA EXPLICACIÓN</div><div className="unlock-summary-grid">{unlocks.map(x=><div className="unlock-summary-item" key={`${x.role}-${x.power}`} style={{'--role-accent':x.accent}}><span>{x.emoji}</span><div><b>{x.power}</b><small>{x.label}</small></div></div>)}</div><p className="muted">La herramienta aparece después de aprender el concepto; ningún equipo obtiene poderes antes que otro.</p></div>
}
    ...
    <div className="grid grid-2 lesson-grid">
      ...
      <div className="card">
        ...
        {facilitator?<ol className="teacher-script">{l.teacher.map(x=><li key={x}>{x}</li>)}</ol>:<>
          ...
          <p><b>Regla:</b> comparte al menos un hallazgo con los otros cuatro roles. Los poderes con candado todavía no se usan.</p>
        </>}
      </div>
