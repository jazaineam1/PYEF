...
export function RoleReveal({state,role:roleProp,round:roundProp,phase:phaseProp='lesson'}){
  ...
  const brief=roleRoundBrief(round,role)
  if(!meta)return null
  ...
  if(phase==='round')return <div className="card" style={{borderColor:meta.accent,marginBottom:14}}>
    ...
    <div style={{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}><span style={{fontSize:34}}>{meta.emoji}</span><div><h3 style={{margin:'0 0 4px'}}>{meta.label}</h3><span><b>Tu responsabilidad:</b> {brief?.deliver||action?.deliver}</span></div></div>
    {brief&&<div className="grid grid-2" style={{marginTop:10}}><div className="notice notice-gold"><b>Pregunta que debes llevar al equipo</b><br/>{brief.ask}</div><div className="notice"><b>Dependes de</b><br/>{brief.needs.map(code=>`${ROLE_ARCHETYPES[code]?.emoji} ${ROLE_ARCHETYPES[code]?.label}`).join(' · ')}</div></div>}
    <div className={`notice ${role==='business'?'notice-gold':'notice-green'}`} style={{marginTop:10}}>{role==='business'?<><b>Autoridad de cierre:</b> tú bloqueas la decisión, pero sólo cuando la Mesa tenga 4/4 evidencias.</>:<><b>No bloqueas la decisión final.</b> Tu trabajo es producir y compartir la evidencia que 🦁 necesita para poder cerrarla.</>}</div>
  </div>
  ...
  return <div className="quest-reveal power-enter" style={{'--active-accent':meta.accent}}>
    ...
    <div className="role-focus-card">
      <div className="role-visual-column"><div className="role-avatar-stage" aria-hidden="true"><i className="role-orbit role-orbit-one"/><i className="role-orbit role-orbit-two"/><span className="role-avatar-glow"/><span className="role-avatar-emoji">{meta.emoji}</span></div><span className="role-visual-caption core">TU ESPECIALIDAD</span></div>
      <div className="role-focus-copy">
        ...
        <div className="grid grid-3" style={{marginTop:12}}><div className="card"><div className="eyebrow">PUEDES</div><p>{action?.can}</p></div><div className="card"><div className="eyebrow">NO PUEDES CONCLUIR SOLO</div><p>{action?.cannot}</p></div><div className="card card-accent"><div className="eyebrow">ENTREGAS AL EQUIPO</div><p><b>{brief?.deliver||action?.deliver}</b></p></div></div>
        {brief&&<div className="grid grid-2" style={{marginTop:12}}><div className="notice notice-gold"><b>En R{round} necesitas preguntar:</b><br/>{brief.ask}</div><div className="notice"><b>Necesitas evidencia de:</b><br/>{brief.needs.map(code=>`${ROLE_ARCHETYPES[code]?.emoji} ${ROLE_ARCHETYPES[code]?.label}`).join(' · ')}</div></div>}
      </div>
    </div>
    ...
    <div className="team-role-strip"><div className="team-role-strip-copy"><b>Interdependencia obligatoria</b><span>{DECISION_RULE.requirement}</span></div><div className="team-role-icons" aria-label="Cuatro especialidades núcleo">{coreRoles.map(m=><i key={m.label} title={m.label} style={{'--role-accent':m.accent}}>{m.emoji}</i>)}</div></div>
  </div>
}
