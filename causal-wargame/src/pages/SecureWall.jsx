...
export default function SecureWall(){
  ...
  useEffect(()=>{if(!code)return;refresh();const i=setInterval(refresh,2500);return()=>clearInterval(i)},[code])
  ...
  return <div className="wall">
    <div className="topbar"><Brand subtitle="Pantalla compartida"/><div style={{display:'flex',gap:8,alignItems:'center'}}><span className="pill">Ronda <strong>{g.round}/4</strong></span><Timer closesAt={g.closes_at?new Date(g.closes_at).getTime():null}/></div></div>
    ...
    {err&&<div className="notice notice-red" style={{marginTop:16}}>{err}</div>}
  </div>
}
