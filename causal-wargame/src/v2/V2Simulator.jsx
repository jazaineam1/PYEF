...
function TeamDecision({round}){
  const payload=SIM_TEAM_DECISIONS[round]
  ...
  if(round===2)return <div className="v2-choice-row">{RECOMMENDATIONS.map(([v,l])=><button type="button" className={payload.recommendation===v?'selected':''} key={v}>{l}</button>)}</div>
  const choices=Object.fromEntries([
    ...(payload.treat||[]).map(x=>[x,'treat']),
    ...(payload.avoid||[]).map(x=>[x,'avoid']),
    ...(payload.observe||[]).map(x=>[x,'observe'])
  ])
  return <><PolicyMeter segments={SIM_SEGMENTS} choices={choices} capacity={SIM_ECONOMY.capacity}/><StudentCards round={3} answer={{choices}}/></>
}

function TeamComparison({round}){
  return <div className="v2-journey-compare">{SIM_PLAYERS.map(p=>{
    const path=SIM_PATHS[p.id][round]
...
function RevealPanel({round,state}){
  const r=V2_ROUNDS[round],result=SIM_REVEALS[round]
  return <div className="v2-reveal compact">
    <div className="v2-kicker">REVEAL · AHORA SÍ APARECE LO OCULTO</div>
    {round===1&&<div className="v2-result money"><b>Valor incremental de la decisión del equipo</b><strong>{cop(result.incremental_value_cop)}</strong><span>Mejor valor posible: {cop(result.best_possible_value_cop)} · brecha {cop(result.value_gap_cop)}</span></div>}
    {round===2&&<div className="v2-metrics"><div><span>Efecto real del mundo simulado</span><strong>+{result.true_effect_pp} pp</strong></div><div><span>Valor potencial</span><strong>{cop(result.campaign_potential_value_cop)}</strong></div></div>}
    {round===3&&<div className="v2-result money"><b>Valor incremental de la política</b><strong>{cop(result.incremental_value_cop)}</strong><span>{result.treated_audience.toLocaleString()} de {result.capacity.toLocaleString()} clientes de capacidad</span></div>}
    <section className="v2-wow"><div className="v2-wow-badge"><Sparkles size={16}/> GIRO</div><strong>{r.wow}</strong><p>{r.takeaway}</p></section>
    <RevealVisuals round={round} state={state} analysis={SIM_ANALYSIS[round]}/>
  </div>
}

function StudentStage({round,phase,student,state}){
  const r=V2_ROUNDS[round]
  const path=SIM_PATHS[student.id][round]
...
export default function V2Simulator(){
  ...
  const teamPool=round===3?SIM_SEGMENTS:SIM_CUSTOMERS
  const state=useMemo(()=>({
    team_pool:teamPool,
    economy:SIM_ECONOMY,
    team_decision:{payload:SIM_TEAM_DECISIONS[round],result:SIM_REVEALS[round]}
  }),[round])
