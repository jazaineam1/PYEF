...
test('bloquea inconsistencias entre contrato y experimento',()=>{
  const rows=[
    row('business',{details:{population:'Clientes elegibles',treatment:'Llamada',comparator:'No llamada',outcome:'Pago completo',horizon:'30 días',estimand:'ATE',constraint:'Capacidad máxima'}}),
...
test('marca coherencia cuando las cinco piezas son compatibles',()=>{
  const rows=[
    row('business',{details:{population:'Clientes elegibles',treatment:'Llamada',comparator:'No llamada',outcome:'Pago completo',horizon:'30 días',estimand:'CATE',constraint:'Capacidad máxima'}}),
    row('data',{details:{estimator:'drlearner'}}),
    row('context',{summary:'Camino bloqueado',details:{diagnosed:true}}),
    row('integrator',{details:{assignment:'random',outcome:'Pago completo',horizon:'30 días'}}),
    row('risk',{details:{selected:['A'],used:2000,capacity:4000,spend:10000,budget:40000,riskViolation:false,feasible:true}})
  ]
