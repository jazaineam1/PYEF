...
test('V2 exposes the causal visual evidence families',()=>{
...
test('visual evidence is integrated between initial and revised decisions before team lock',()=>{
  const src=read('src/v2/V2Play.jsx')
  assert.match(src,/DecisionSummary state=\{state\}/)
  assert.match(src,/EvidenceLab round=\{round\}/)
  assert.match(src,/send\('revision'/)
