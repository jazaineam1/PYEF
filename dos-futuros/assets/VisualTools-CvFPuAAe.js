...
print("
Esto evalúa predicción de renovación, no efecto de intervenir.")`},{id:`predictive-rank`,label:`2 · Rankear`,question:`Si el modelo predice bien, ¿a quién pondría arriba del ranking?`,instruction:`Ajusta el modelo con todo el histórico y predice las 24 cohortes disponibles.`,code:`import pandas as pd
from sklearn.linear_model import LogisticRegression
...
print("
Pregunta:")
print("¿Qué variable de esta tabla dice cuánto CAMBIARÍA la renovación por intervenir?")`},{id:`predictive-visual`,label:`3 · Diagnosticar`,question:`¿Probabilidad alta significa impacto alto?`,instruction:`Visualiza la probabilidad predictiva. Nota qué información todavía no existe en este dataset.`,code:`import pandas as pd
...
print("No observamos todavía Y(1)-Y(0), por lo que no podemos rankear efecto causal.")`}]}:e===2?{title:`Reconstruye la comparación causal`,context:{observacional:v(r.observational_rows||[])},steps:[{id:`crude`,label:`1 · Crudo`,question:`¿Qué concluye una comparación directa entre tratados y no tratados?`,instruction:`Calcula la diferencia observada y revisa quién recibió tratamiento.`,code:`import pandas as pd
...
print("
Antes de llamarlo efecto, pregunta por qué cambió tanto la probabilidad de tratamiento según riesgo.")`},{id:`propensity-ipw`,label:`2 · Ajustar`,question:`¿Qué ocurre cuando modelamos la asignación y reponderamos observaciones comparables?`,instruction:`Estima propensity score con regresión logística y un ATE por IPW estabilizado.`,code:`import pandas as pd
import numpy as np
from sklearn.linear_model import LogisticRegression
...
print("
El ajuste cambia la comparación; no convierte automáticamente los supuestos en hechos.")`},{id:`overlap`,label:`3 · Overlap`,question:`¿Hay soporte común suficiente para construir contrafactuales?`,instruction:`Grafica propensity scores de tratados y controles y revisa las zonas con poca superposición.`,code:`import pandas as pd
import numpy as np
import matplotlib.pyplot as plt
from sklearn.linear_model import LogisticRegression
...
print("
Donde casi sólo existe un grupo, la estimación depende más de extrapolación.")`}]}:e===3?{title:`Del experimento a una política`,context:{experimento:y(r.experiment_rows||[]),segmentos:b(r.segments||[]),economia:r.economy||t.economy||{}},steps:[{id:`ate-ci`,label:`1 · ATE`,question:`¿Cuál es el efecto promedio del experimento y cuánta incertidumbre tiene?`,instruction:`Estima diferencia de medias e intervalo de confianza del experimento aleatorizado.`,code:`import pandas as pd
import numpy as np
...
print("p-value    :", round(stats.ttest_ind(y1,y0,equal_var=False).pvalue,4))`},{id:`t-learner`,label:`2 · CATE`,question:`¿El efecto promedio esconde respuestas diferentes entre segmentos?`,instruction:`Entrena un T-Learner real: un modelo para tratados y otro para controles.`,code:`import pandas as pd
...
print("
ATE promedio del T-Learner:", round(cate.reindex(df["segmento"]).mean(),1), "pp")`},{id:`policy-value`,label:`3 · COP`,question:`¿Qué segmentos generan valor incremental después de considerar costo y capacidad?`,instruction:`Convierte el CATE estimado en COP. El resultado es una ayuda para decidir, no una regla automática.`,code:`import pandas as pd
from sklearn.ensemble import RandomForestRegressor
import matplotlib.pyplot as plt
